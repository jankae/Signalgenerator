	...
	for(uint8_t i=0;i<maxEntries;i++) {
		if(entries[i].ptr) {
			...
			if (freeOffset + size <= usableSize) {
				...
				CRIT_ERROR("Unable to add data to persistence, exeeded size");
				return false;
			}
		}
	}
	CRIT_ERROR("Unable to add data to persistence, exeeded number of entries");
	return false;
}
...
bool Persistence::Save() {
	...
	uint32_t crc = common_crc32(0, (uint8_t*) start, usableSize);
...
bool Persistence::Load() {
	...
	uint32_t crc = common_crc32(0, start, usableSize);
